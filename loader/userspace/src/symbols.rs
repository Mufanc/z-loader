...
pub fn resolve<P : AsRef<Path>>(library: P, name: &str) -> Result<u64> {
    let data = fs::read(library)?;
    resolve_internal(&data, name, None)
}

fn resolve_internal(data: &[u8], name: &str, mirror: Option<&File>) -> Result<u64> {
    ...
    if let Some(section) = object.section_by_name(".gnu_debugdata") {
        ...
        return resolve_internal(&inner, name, Some(&object));
    }
